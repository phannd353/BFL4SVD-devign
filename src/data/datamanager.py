...
def load(path, pickle_file, ratio=1):
    ...
    if "input" in dataset:
        dataset["input"] = dataset["input"].map(_upgrade_pyg_data)
...
def _upgrade_pyg_data(data):
    if isinstance(data, Data) and "_store" not in data.__dict__:
        attributes = {
            key: value
...
def train_val_test_split(data_frame: pd.DataFrame, shuffle=True):
    ...
    test_false, val_false = train_test_split(
        test_false, test_size=0.5, shuffle=shuffle, random_state=2020
    )
    train_true, test_true = train_test_split(
        true, test_size=0.2, shuffle=shuffle, random_state=2020
    )
    test_true, val_true = train_test_split(
