...
def tokenizer(code, flag=False):
    ...
    if flag:
        # C source is not a Python string literal; decoding it as
        # ``unicode_escape`` fails on valid/incomplete sequences such as
        # ``\x`` found in macros or malformed source text.
        code = no_char_lit_line.replace("\\n", "\n").replace("\\r", "\r")
